...
This is a skill development project within the Skills Factory workspace. Its goal is to make Claude Code the best frontend builder by creating skills and configuring MCP servers that eliminate component hallucination, enforce design systems, and verify accessibility.

The plan lives in `frontend-skill-mcp-system-plan.md` — read it before starting any implementation work.

## What This Project Produces

The deliverables are **not code in this repo** — they are files installed into the user's global Claude Code config (`~/.claude/`):

1. **shadcn MCP + Figma MCP** — added to `~/.claude/.mcp.json`
2. **Frontend Production Standards** — added as a section in `~/.claude/CLAUDE.md`
3. **`design-system-enforcer` skill** — created at `~/.claude/skills/design-system-enforcer/SKILL.md`
4. **`a11y-audit` skill** — created at `~/.claude/skills/a11y-audit/SKILL.md`

## Parent Project Rules

This project inherits the Skills Factory methodology (see `../CLAUDE.md` and `../SKILL-CRAFT.md`):

- Follow the SKILL-CRAFT methodology for all skill creation
- Maintain a `skill-development-log.md` for each skill being developed
- Skills should transfer expertise, not just list instructions
- Voice should match domain practitioners, not documentation

## Key Architectural Decisions

These are already settled in the plan — don't revisit them:

- **No separate Accessibility MCP** — Playwright's `browser_snapshot` (accessibility tree) replaces it
- **No ScreenshotOne MCP** — Playwright's `browser_take_screenshot` covers this
- **No PostToolUse a11y hook** — shell hooks can only echo text; a11y checks belong in skill workflows
- **No separate `figma-to-code` skill** — merged into `design-system-enforcer` as a conditional path
- **Production rules go in CLAUDE.md, not a skill** — always-loaded beats must-be-triggered

## Implementation Phases

1. **MCP Foundation** — Install shadcn + Figma MCP servers, verify Playwright a11y
2. **CLAUDE.md Standards** — Add Frontend Production Standards section
3. **Skills** — Create `design-system-enforcer` and `a11y-audit` skills
4. **Validation** — End-to-end testing with real frontend tasks

## MCP Servers

- **shadcn** — enabled in `.claude/settings.local.json`; provides live component registry queries
- **Figma** — requires `FIGMA_TOKEN` env var; provides design context extraction
- **Playwright** — already globally installed; used for a11y tree, screenshots, and visual verification
...
- **Notion DB Data Source ID:** 16a3924f-4651-490d-817d-e6335ce6566c
- **Default View URL:** https://www.notion.so/9efa88d224674a68a3b778670b200d6c
...
**Reading the Roadmap:**
```
notion-query-database-view with view_url: "https://www.notion.so/9efa88d224674a68a3b778670b200d6c"
```

**Creating items:**
```
notion-create-pages with parent: { data_source_id: "16a3924f-4651-490d-817d-e6335ce6566c" }
properties: {
  "Item": "Description",
  "Status": "Insight",
...
- CLAUDE.md
- frontend-skill-mcp-system-plan.md
